    ...
    <div style={{ padding: '32px 36px', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 32 }}>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: 'Jura', fontSize: 12, fontWeight: 700, color: C.greenLight, textTransform: 'uppercase', letterSpacing: 2, marginBottom: 12 }}>
            Jakarta's New Way to
          </div>
          <div style={{ fontFamily: 'Jura, sans-serif', fontWeight: 800, fontSize: 38, color: '#fff', lineHeight: 1.15, marginBottom: 14, position: 'relative' }}>
            Find your<br />
            <span style={{ color: C.yellowMid }}>tribe</span> &amp;<br />
            <span style={{ color: C.greenLight }}>move</span> together.
          </div>
          <div style={{ fontFamily: 'Jura', fontSize: 14, color: C.greenLight, marginBottom: 24, lineHeight: 1.7, position: 'relative' }}>
            Events with smart gender quotas, zero commission, and a community that shows up.
          </div>
          <div style={{ display: 'flex', gap: 10, position: 'relative' }}>
            <button
              onClick={() => setPage('discover')}
              style={{ padding: '11px 24px', borderRadius: 10, background: C.yellowPale, color: C.greenDark, fontFamily: 'Jura', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer' }}
            >
              Explore Events
            </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {[['500+', 'Events Live', C.yellowDeep], ['12K+', 'Members', C.greenSage], ['0%', 'Commission', C.greenSage]].map(([n, l, col]) => (
              <div key={l} style={{ background: C.bgCard, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: '16px 12px', textAlign: 'center' }}>
                <div style={{ fontFamily: 'Jura, sans-serif', fontWeight: 800, fontSize: 24, color: col }}>{n}</div>
                <div style={{ fontFamily: 'Jura', fontSize: 11, color: C.textMuted, marginTop: 3 }}>{l}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: C.bgCard, border: `1.5px solid ${C.border}`, borderRadius: 16, padding: 20, flex: 1 }}>
            <div style={{ fontFamily: 'Jura, sans-serif', fontWeight: 800, fontSize: 17, color: C.text, marginBottom: 6 }}>
              ⚧ Our Signature Feature
            </div>
            <div style={{ fontFamily: 'Jura', fontSize: 13, color: C.textMuted, marginBottom: 16, lineHeight: 1.7 }}>
              ...
              {[['♀ 8 slots', C.female], null, ['♂ 12 slots', C.male], null, ['= 20 total', C.greenSage]].map((item, i) =>
                item === null
                  ...
                  : <div key={i} style={{ padding: '10px 8px', background: item[1] + '14', borderRadius: 10, textAlign: 'center' }}>
                      <div style={{ fontFamily: 'Jura, sans-serif', fontWeight: 800, fontSize: 15, color: item[1] }}>{item[0]}</div>
                    </div>
              )}
            </div>
            <div style={{ marginTop: 12, fontFamily: 'Jura', fontSize: 11, color: C.textMuted, fontStyle: 'italic' }}>
      ...
      {/* ── Categories ── */}
      <div style={{ fontFamily: 'Jura, sans-serif', fontWeight: 800, fontSize: 20, color: C.text, marginBottom: 16 }}>
        Browse by Category
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 32 }}>
        {CATEGORIES.slice(1).map(c => (
          <button
            key={c}
            onClick={() => setPage('discover')}
            style={{ padding: '8px 16px', borderRadius: 99, fontFamily: 'Jura', fontWeight: 600, fontSize: 13, cursor: 'pointer', background: C.bgCard, color: C.textMid, border: `1.5px solid ${C.border}`, transition: 'all .18s' }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {events.slice(0, 3).map(e => (
          ...
          >
            ...
            <div style={{ fontFamily: 'Jura, sans-serif', fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 6, lineHeight: 1.3 }}>
              {e.title}
            </div>
            <div style={{ fontFamily: 'Jura', fontSize: 12, color: C.textMuted, marginBottom: 10 }}>
              📅 {e.date} · 📍 {e.location.split(',')[0]}
            </div>
            <PublicSlotBar filled={e.totalFilled} total={e.totalSlots} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}>
              <span style={{ fontFamily: 'Jura, sans-serif', fontWeight: 800, fontSize: 16, color: C.yellowDeep }}>
